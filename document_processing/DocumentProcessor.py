...
class DocumentProcessor:
    ...
    def set_image_page_descriptions(self, descriptions: List[str]) -> None:
        ...
        for page in self.pages:
            page_description = "Text: " + page.text.content + "\n".join([f"Image{i}: {image.description}" for i, image in enumerate(page.images, start=1)])
    ...
    def get_embedding_records(self) -> Dict[str, Dict[str, List[Any]]]:
        ...
        record_keys_mapping = {"ids": "id", "documents": "document", "metadatas": "metadata", "embeddings": "embedding"}
        ...
        for page in self.pages:
            for records_key, record_key in record_keys_mapping.items():
                ...
                # Text-level records
                records["text"][records_key].append(page.text.record[record_key])

            # Image-level records
            for image in page.images:
                for records_key, record_key in record_keys_mapping.items():
                    records["image"][records_key].append(image.record[record_key])
